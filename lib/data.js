...
function normalizeRooms(rooms) {
  ...
  for (const room of rooms) {
    ...
    const floor_id =
      id === "garden" || id === "car_park"
        ? "ground_floor"
        : room.floor_id === "exterior_site"
          ? "ground_floor"
          : room.floor_id;

    const images = [
      ...new Set([...(existing?.images ?? []), ...(Array.isArray(room.images) ? room.images : [])]),
    ];
    ...
    byId.set(id, {
      ...
      floor_id,
      images,
      ...(existing?.ownership && !room.ownership ? { ownership: existing.ownership } : {}),
    });
  }
...
export function getData() {
  ...
  const rooms = normalizeRooms(loadYaml("rooms.yaml"));
